...
FxEmbed — the worker behind FxTwitter / FixupX / FxBluesky / FixTok / FxInstagram. It rewrites social post URLs into rich embeds for Discord, Telegram, etc., and exposes a public JSON API. TypeScript on Cloudflare Workers, Hono routing, zod/`@hono/zod-openapi` validation, i18next (+ICU) localization, esbuild bundling, Vitest under Miniflare.
...
| Single test file | `npx vitest run test/api.status.test.ts` |
...
The single worker serves many domains. `app`'s custom `getPath` inspects the request hostname and **rewrites the path to `/<realm>/<pathname>`** before Hono matches routes — `api`, `blueskyapi`, `atmosphere`, `twitter`, `bluesky`, `tiktok`, `instagram`. Realm is chosen from the `*_HOST_LIST` / `STANDARD_*_DOMAIN_LIST` constants, defaulting to `twitter` for unknown domains; on `workers.dev`/`localhost` the realm prefix is left off so every realm is reachable by path.

Consequence for local testing: **the `Host` header decides behavior.** `curl -H "Host: fxtwitter.com" http://localhost:8787/user/status/123`, or hit `/twitter/...`, `/api/...` etc. directly. Embed HTML only renders for a bot UA (`Constants.BOT_UA_REGEX`, e.g. `Discordbot/2.0`); otherwise the worker 302s to the original platform.
...
`src/realms/<realm>/router.ts` (route table + realm quirks) → `src/realms/<realm>/routes/*.ts` (parses URL/host/UA into `InputFlags`) → `src/embed/status.ts` `handleStatus` (the shared embed pipeline: fetch via provider, localize, build thread) → `src/render/{photo,video,instantview}.ts` (emit meta-tag instructions) → `Strings.BASE_HTML`.

Behavior is driven heavily by **subdomain flags** resolved in the route layer from `Constants` domain lists: `d.` direct media, text-only, gallery, force-mosaic, instant view, old-embed. Adding a new behavior generally means a new domain list env var + a flag in `InputFlags`.
...
The package must stay free of Hono/worker specifics. The worker keeps: Hono routing, OpenAPI wiring, branding, and credentials. Where a provider needs host context, the worker passes an adapter (`src/providers/*/build-host-adapter.ts`) or registers routes (`src/providers/*/atmosphere-register.ts`).

### API realms

`src/realms/api` is an `OpenAPIHono` app. v2 routes are declared in `routes.ts` (zod schemas in `schemas.ts`) and bound in `router.ts`; the spec is served from `/2/openapi.json`. Legacy v1 endpoints reuse the Twitter embed handlers. The `atmosphere` realm mounts the same in-process `api` and `blueskyApi` apps plus multi-provider routes. All API realms require a `User-Agent` header (401 otherwise).
